...
####################### 3rd reviewer
####################### Pondscape and country B-EMF relationship (scale comparison)
#######################
setwd("~/Dropbox/PONDERFUL/Database/data/Ecosystem functions/Analisis/All/SEM/Article/Documents to submit/Database_Script")
...
#######################
TF <- read_excel("~/Dropbox/PONDERFUL/Database/data/Ecosystem functions/Analisis/All/SEM/Article/R1_11Sep2025/NewAnalysis/R_scripts/EMF_rawDataT.xlsx")
model<-lme(EMF ~ MDiv * Ard, #substitute MDiv by the different diversities (BioMP, BioZP, BioMI, BioAmph)
...
####################### 3rd reviewer
####################### To normalize variables - Z-Score Vs Max-Min normalization
#######################
#set up your tables
...
# 1. Load your data (copy-paste as a dataframe)
data <- read_excel("CHLa analsys.xlsx")
data <- data %>%filter(!is.na(Chla_Spectro_predicted)) #Remove missing values
data <- data %>% mutate(Bias = Chla_Spectro_predicted - Chla_Spectro) #Calculate bias
data %>% group_by(Instrument) %>% summarise(mean_bias = mean(Bias),sd_bias = sd(Bias),n = n())#Summary statistics

#Statistical test
model <- lm(Bias ~ Instrument, data = data)
summary(model)
model <- lm(Chla_Spectro_predicted ~ Chla_Spectro * Instrument, data = data)
summary(model)

# Plotting
ggplot(data, aes(x = Instrument, y = Bias)) + geom_boxplot() + geom_jitter(width = 0.1, alpha = 0.5) +
  theme_minimal() + labs(y = "Bias (Instrument - Spectrophotometry)")
ggplot(data, aes(x = log(Chla_Spectro), y = log(Chla_Spectro_predicted), color = Instrument)) +
  geom_point() + geom_smooth(method = "lm", se = FALSE) +
  theme_minimal() + labs(x = "Spectrophotometry", y = "Instrument")

#******
data <- data %>% mutate(log_Spectro = log10(Chla_Spectro + 0.001),log_Predicted = log10(Chla_Spectro_predicted + 0.001))

# Compute R² per country
r2_labels <- data %>%
  group_by(Instrument) %>%
  summarise(r2 = summary(lm(log_Predicted ~ log_Spectro))$r.squared) %>%
  mutate(label = paste0(Instrument, ": R² = ", round(r2, 3)))

# Build labels with equation, R², and p-value for each country
ggplot(data, aes(x = log_Spectro, y = log_Predicted, color = Instrument)) +
  geom_point(alpha = 0.7) + geom_smooth(method = "lm", se = FALSE) +
  geom_text(data = r2_labels,aes(x = min(data$log_Spectro), y = max(data$log_Predicted) - (as.numeric(factor(Instrument)) * 0.3),
        label = label, color = Instrument), hjust = 0, inherit.aes = FALSE) + theme_minimal() 

slope_test <- lm(log_Predicted ~ log_Spectro * Instrument, data = data)
anova(slope_test)

#****************************************************************************R3
####################### 3rd reviewer
####################### Year and aridity comparison
####################### Supplementary table 6

#Including the interaction between MDiv and both terms, AI and Year
TF <- read_excel("~/Dropbox/PONDERFUL/Database/data/Ecosystem functions/Analisis/All/SEM/Article/R1_11Sep2025/NewAnalysis/R_scripts/EMF_rawDataT.xlsx")
model<-lme(EMF ~ MDiv * AI + MDiv * Year, #substitute MDiv by the different diversities (BioMP, BioZP, BioMI, BioAmph)
           random = list(~ Country|Pondscape), #Also tested: random = list(~ Country|Pondscape, ~ Month|Year),
           control=list(msMaxIter=200,opt = "optim",msVerbose=TRUE),
           #weights = varIdent(form = ~ 1|Year), #Specifies that the residual variance differs across different months within each year. This allows for modeling heteroscedasticity.
           na.action = na.omit,
